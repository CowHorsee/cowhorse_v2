...
export default function ManagerApprovalPage() {
  ...
  useEffect(() => {
    ...
    if (user?.role !== 'MANAGER' && user?.role !== 'ADMIN') {
      void router.replace(
        prId ? `/pr/details?id=${encodeURIComponent(prId)}` : '/pr'
      );
    }
  }, [prId, router]);
  ...
  async function handleDecision(nextDecision: 'APPROVED' | 'REJECTED') {
    const sessionUser = getUserSession();
    if (!purchaseRequest || !sessionUser?.user_id) {
      return;
    }
    ...
    try {
      await reviewPurchaseRequest({
        ...
        manager_id: sessionUser.user_id,
      });
      ...
      setErrorMessage(
        error instanceof ApiError
          ? error.message
          : 'Unable to record the decision right now.'
      );
    } finally {
    ...
    return (
      <div className="mx-auto w-full max-w-7xl">
        <Card variant="surface" padding="lg">
          <p className="text-sm text-slate-500">Loading approval request...</p>
        </Card>
      </div>
    );
  }
      ...
      <Card variant="surface" padding="lg">
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex items-center rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-80"
          >
            Approve
          </button>
          <button
            ...
            className="inline-flex items-center rounded-lg bg-brand-red px-4 py-2 text-sm font-bold text-brand-white transition hover:bg-[#ad2d2d] disabled:cursor-not-allowed disabled:opacity-80"
          >
            Reject
          </button>
        </div>
